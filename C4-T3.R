...
iphone_matrix <- tbl_df(iphone_matrix)
...
# extracting columns related to iPhone and Samsung without a dependend variable (iPhoneSentiment)
...
# feature selection
# 1 method: running Random Forest or other decision tree and check which varables are used: predictors()
...
# poor results even with different samples (1500, 3000 and 4000)
# predictor variables aren't removed and the result suggest overfiting ()
# mtry  Accuracy   Kappa    
...
iphone_sample_C5.0
predictors2 <- predictors(iphone_sample_C5.0)
predictors2
length(predictors2)
# leaves 20 predictors, choosen for the further analysis
...
# data partition: first method of the feature selection (based on decision tree) 
set.seed(222)
...
# SVM: 1 method, 21 variables, first try on a full set: overfitting 
...
# second method of the feature selection
# findCorrelation() evaluates the relationships within the dataset and creates a list of highly correlated predictors for elimination 
# data can't be discretized

# corelations identified in the whole dataset and than removed from sample 
...
# discretizing the dependent variable (iphoneSentiment)
...
# modeling: iphone and samsung (the best result: RF)
...
# C5.0 - problems with the algorithm     
